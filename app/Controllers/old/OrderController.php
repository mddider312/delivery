...
class OrderController extends BaseController
{
	...
	{
		$data = [
			'title_meta' => view('partials/title-meta', ['title' => 'Order List']),
			'page_title' => view('partials/page-title', ['title' => 'Order List', 'li_1' => 'Dashboard', 'li_2' => 'Order List']),
	...
	public function update_order_status() {
	    ...
	    $result = $this->orderModel->set(['status' => $order_status])->where('order_id', $order_id)->update();
	...
	public function find_claimable_product() {
	    $product_id = $this->request->getPost('product_id');
	    $partner_id = $this->request->getPost('partner_id');
        
        $claimable = $this->transactionProductModel->where(["partner_id" => $partner_id, "product_id" => $product_id, "order_id" => 0, "status" => 0, "transaction_status" => 1])->findAll();
        
        echo json_encode($claimable);
	}
	
	public function submit_claim() {
	    $otp_id = $this->request->getPost('order_transaction_product_id');
	    $old_id = $this->request->getPost('old_transaction_product_id');
	    $new_id = $this->request->getPost('new_transaction_product_id');
	    $claim_reason = $this->request->getPost('claim_reason');
	    $claim_at = $this->datetime;
	    
	    $claim_reason = "ID $old_id -> $new_id<br>($claim_at)<br>$claim_reason";
	    
	    $result1 = $this->db->query("UPDATE bs_orders_transactions_products SET transaction_product_id = '$new_id', is_claim = 1, claim_reason = '$claim_reason', claim_at = '$claim_at' WHERE order_transaction_product_id = '$otp_id'");
	    
	    if ($result1) {
	        $old = $this->db->query("SELECT * FROM bs_transactions_products WHERE id = '$old_id'")->getResultArray();
	        $order_id = $old[0]['order_id'];
	        $order_product_id = $old[0]['order_product_id'];
	        $sold_at = $old[0]['sold_at'];
	        
	        $result2 = $this->db->query("UPDATE bs_transactions_products SET order_id = '$order_id', order_product_id = '$order_product_id', status = 1, sold_at = '$sold_at' WHERE id = '$new_id'");
	        
	        if ($result2) {
	            $result3 = $this->db->query("UPDATE bs_transactions_products SET order_id = 0, order_product_id = 0, status = 0, sold_at = null WHERE id = '$old_id'");
