...
class CustomerController extends BaseController
{
    public function __construct() {
        ...
        // WhatsApp Message Blasting
        $this->whatsappToken = "EAAH8DRCQ1pIBANXu1sSNY3AYnaSsWAXuuOKcK4xnnxTq33bDJzqfqqYFxpzBZCCbZCFc2zHgqZAXya6Ug04QiZCIB335II89giWS4N6aJ6dFa48mI6QLLnHGNelGAtRZAuIS0J7duEYcNVoNXZBcZAffxsMqrApTJSaZBJDW8namH7Ym7rGoZCwRFkE0JKzR8mbcsAEqZBh1ex1AZDZD";
        $this->whatsapp_url = "https://graph.facebook.com/v15.0/108835585425659/messages";
        ...
        */
        ...
        // Bateri Laju Sdn Bhd
        /*
        $this->api_key = 'c1781133-9d2a-487e-975a-e2a918f887a7';
        $this->collection_id = 'tswzxpur';
        $this->x_signature = 'S-ynqyvLrmJZ6RjvhSo8Pz5w';
        */
        
        // Revive Battery Shop Sdn Bhd
        
        $this->api_key = 'c6909f41-a6f2-4cd1-aba4-9ab1000bc19d';
        $this->collection_id = 'nzeqerkt';
        $this->x_signature = 'S-AZ_isb72fEGfpTDVF4l_xg';
	...
	public function billplz($order_id) {
        ...
        /***********************************************/
        if ($rheader !== 200) {
            echo json_encode(["status" => "failed", "billplz_bill_link" => "", "order_id" => 0, "trade_in_id" => 0, "payment_method" => $payment_method]);
            ...
            echo json_encode(["status" => "success", "billplz_bill_link" => $rbody['url'], "order_id" => $order_id, "trade_in_id" => $trade_in_id, "payment_method" => $payment_method]);
        }
	...
	public function payment_result($order_id, $trade_in_id, $partner_id, $workshop_id) {
	    ...
	    if ($paid == "true") {
	        //add commission, send whatsapp message and redirect link here
	...
	public function place_order() {
        ...
        if ($condition == "1") { // tradein only
    		...
    		if ($result) {
    		    echo json_encode(["status" => "success", "billplz_bill_link" => "", "order_id" => 0, "trade_in_id" => $trade_in_id, "payment_method" => $payment_method]);
    		} else {
    		    echo json_encode(["status" => "failed", "billplz_bill_link" => "", "order_id" => 0, "trade_in_id" => 0, "payment_method" => $payment_method]);
    		}
        }
        ...
        if ($condition == "2") { // order only
    		...
    		if ($result) {
    		    if ($final_total <= 0.00) {
    		        echo json_encode(["status" => "success", "billplz_bill_link" => "", "order_id" => $order_id, "trade_in_id" => 0, "payment_method" => $payment_method]);
    		    } else {
        ...
        if ($condition == "3") { // tradein and order
            $data = [
    			...
    			'status'		    => 0,
    			'payment_status'    => $final_total <= 0.00 ? 0 : 2,
    			'payment_method'    => $final_total <= 0.00 ? 0 : $payment_method,
    			'created_at'        => $this->datetime,
    		];
    		...
    		if ($result) {
    		    if ($final_total <= 0.00) {
    		        echo json_encode(["status" => "success", "billplz_bill_link" => "", "order_id" => $order_id, "trade_in_id" => $trade_in_id, "payment_method" => $payment_method]);
    		    } else {
    		        switch ($payment_method) {
    		            case "0": // Not applicable
    		            case "2": // Cash
	...
	public function load_checkout_detail() {
	    ...
	    if ($order_id != 0) {
	        ...
	        $payment_method = $order['payment_method'] == 0 ? 'Not applicable' : ($order['payment_method'] == 1 ? 'Bank' : 'Cash');
        ...
        $this->orderTradeInDetailModel->insert([
            "order_id" => $order_id ?? 0,
            "trade_in_id" => $trade_in_id ?? 0,
            "unique_code" => $unique_code,
            "html" => $html,
            "created_at" => $this->datetime,
        ]);
        
        $this->send_whatsapp_message($order_id, $trade_in_id);
        
        echo $html;
	}
	
	public function send_whatsapp_message($order_id, $trade_in_id) {
	    // Send Whatsapp Start
	    $order = $this->orderModel->where("order_id", $order_id)->findAll();
	    $tradein = $this->tradeInModel->where("trade_in_id", $trade_in_id)->findAll();
        $customer_name = COUNT($order) != 0 ? $order[0]['customer_name'] : $tradein[0]['customer_name'];
        $customer_phone = COUNT($order) != 0 ? $order[0]['customer_phone'] : $tradein[0]['customer_phone'];
        
        $detail = $this->orderTradeInDetailModel->where(["order_id" => $order_id, "trade_in_id" => $trade_in_id])->findAll();
        $unique_code = $detail[0]['unique_code'];
        $link = base_url()."/order/".$unique_code;
        ...
        $title = "";
        $thank = "";
        if ($order_id != 0 && $trade_in_id != 0) {
            $title = "order & trade-in";
            $thank = "purchase";
        } else if ($order_id != 0) {
            $title = "order";
            $thank = "purchase";
        } else if ($trade_in_id != 0) {
            $title = "trade-in";
            $thank = "trade-in";
        }
                ...
                array(
                    array(
                        ...
                        "parameters" => array(
                            ...
                            array("type"=> "text","text"=> "$link"),
                            array("type"=> "text","text"=> "$thank")
                        )
                    )
                )
            )
    ...
    public function show_order_detail($unique_code) {
         $data = [
			'title_meta' => view('partials/title-meta', ['title' => "Order Detail"]),
			'page_title' => view('partials/page-title', ['title' => "Order Detail", 'li_1' => 'Order Detail']),
			'order' => $this->orderTradeInDetailModel->where('unique_code', $unique_code)->findAll(),
		];
